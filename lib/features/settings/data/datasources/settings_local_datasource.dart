...
class SettingsLocalDatasourceImpl implements SettingsLocalDatasource {
  ...
  Future<domain.AppSettings> getSettings() async {
    ...
    return domain.AppSettings(
      ...
      enableNotifications:
          _prefs.getBool('${_prefix}enableNotifications') ?? true,
      autoDeleteAfterUpload:
          _prefs.getBool('${_prefix}autoDeleteAfterUpload') ?? false,
      maxConcurrentUploads:
          _prefs.getInt('${_prefix}maxConcurrentUploads') ?? 3,
      uploadQuality: domain.UploadQuality.values[
          _prefs.getInt('${_prefix}uploadQuality') ??
              domain.UploadQuality.original.index],
      ...
      requireWifiForUpload:
          _prefs.getBool('${_prefix}requireWifiForUpload') ?? false,
      enableAutoBackup: _prefs.getBool('${_prefix}enableAutoBackup') ?? false,
      baseDownloadUrl: _prefs.getString('${_prefix}baseDownloadUrl') ??
          'https://project.ibartstech.com', // New field
    );
  }
  ...
  Future<void> updateSettings(domain.AppSettings s) async {
    ...
    await _prefs.setBool(
        '${_prefix}enableNotifications', s.enableNotifications);
    await _prefs.setBool(
        '${_prefix}autoDeleteAfterUpload', s.autoDeleteAfterUpload);
    await _prefs.setInt(
        '${_prefix}maxConcurrentUploads', s.maxConcurrentUploads);
    ...
    await _prefs.setBool(
        '${_prefix}requireWifiForUpload', s.requireWifiForUpload);
    await _prefs.setBool('${_prefix}enableAutoBackup', s.enableAutoBackup);
    await _prefs.setString(
        '${_prefix}baseDownloadUrl', s.baseDownloadUrl); // New field
  }
}
